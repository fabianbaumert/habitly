...
final habitHistoryProvider = FutureProvider.family<Map<String, bool>, DateTime>((ref, date) async {
  ...
  try {
    ...
    // Check if online and we should check for newer data only if local data is empty
    final isOnline = ref.read(isOnlineProvider);
    
    if (isOnline && localData.isEmpty) {
      try {
        // Try to get data from Firestore only if we don't have local data
